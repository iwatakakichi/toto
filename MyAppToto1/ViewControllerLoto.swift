...
class ViewControllerLoto: NSViewController {
    ...
    @IBOutlet weak var nextQuickButton: NSButton!
    ...
    @objc func update(tm: Timer) {
        ...
        if lotoManage.quickPick == Array<Int>(repeating: on,count:5) {
            ...
            carMatrix.cells[carMatrix.cells.count - 1].image = NSImage(named: NSImage.Name("car2"))
        ...
        if lotoManage.category == 0 || lotoManage.category == 1{
            for row in 0...4{
                for cell in lotoManage.markOfloto...6{
                    ...
                    markBallMatrix.cells[seq].image =  NSImage(named: NSImage.Name(""))
                }
            }
        }
        for row in 0...lotoManage.conditionsSet[0].count - 1{
    ...
    private func display(_ completionHandler: () -> Void) {
        ...
        lotoManage.overalls[lotoManage.category] = lotoManage.rakutenData.count - 1 //edit 前回データ除く。
        ...
        if lotoManage.overalls[lotoManage.category] > 0 {
             for i in 1...lotoManage.overalls[lotoManage.category]{  //edit全部 但し、前回データ除く。
                for j in 2...mark + 1{
    ...
    @IBAction func keyPadAction(_ sender: Any) {
        ...
        if push == 46{
            ...
            switch lotoManage.tab {
            case 0:
                numericKeypadMatrix.cells[46].image = NSImage(named: NSImage.Name("redPencil"))
            case 1:
                numericKeypadMatrix.cells[46].image = NSImage(named: NSImage.Name("blackPencil"))
            default:
                print("error")
            }
        }
    ...
    @IBAction func sliderAction(_ sender: Any) {
        ...
        let a: Double = Double((100 - mySlider.integerValue)) * 0.01
        lotoManage.cpuTimeInterval = a * a * a * a * a
        if lotoManage.cpuTimeInterval == 0{
            lotoManage.cpuTimeInterval = 0.00001
        }
        UserDefaults.standard.set(lotoManage.slider, forKey: "slider")
        if lotoManage.drivingMode == on {
            timer.invalidate()
            lotoManage.distribution.sort { $0[0] < $1[0] }
            timer = Timer.scheduledTimer(timeInterval: lotoManage.cpuTimeInterval, target: self, selector: #selector(self.drivingCar), userInfo: nil, repeats: true)
            timer.fire()
        }
    }
    ...
    func chanceRecord(inDt: Int) -> Void {
        ...
        for row in 0...rowSize - 1 {
            ...
            //edit  しばらく、前回当選番号を除外数字にする。のちにfor文カット
            for cell in 0...lotoManage.markOfloto - 1{
        ...
        case dm.normal.rawValue:
            for key in 0...lotoManage.sizeOfloto - 1{
                if lotoManage.layer[ly.redBollot.rawValue][key] == on{
                    numericKeypadMatrix.cells[key].image = NSImage(named: NSImage.Name("blackBollot"))
                    numericKeypadMatrix.cells[key].title = String(lotoManage.keyboard[key])
                }
            }
            for key in 0...lotoManage.sizeOfloto - 1{
                if lotoManage.layer[ly.redCircle.rawValue][key] == on{
                    numericKeypadMatrix.cells[key].image = NSImage(named: NSImage.Name("redCircle"))
                    numericKeypadMatrix.cells[key].title = String(lotoManage.keyboard[key])
                }
            }
        case dm.frequentNumbers.rawValue:
                        ...
                        if lotoManage.keyboard[key] == no{
                            for cell in 0...lotoManage.sizeOfloto - 1{
                                if no == lotoManage.keyboard[cell]{
                                    numericKeypadMatrix.cells[cell].image = NSImage(named: NSImage.Name("blackBollot"))
                ...
                if lotoManage.layer[ly.redCircle.rawValue][layer] == on{
                    ...
                    for key in 0...lotoManage.sizeOfloto - 1{
                        if lotoManage.keyboard[key] == no{
                            for cell in 0...lotoManage.sizeOfloto - 1{
                                if no == lotoManage.keyboard[cell]{
                                    numericKeypadMatrix.cells[cell].image = NSImage(named: NSImage.Name("redCircle"))
                                    numericKeypadMatrix.cells[cell].title = String(lotoManage.keyboard[cell])
                                }
                            }
                        }
    ...
    func setShuffleNumber(inDt:Int){
        ...
        for colum in 0...lotoManage.markOfloto - 1{
            ...
            if lotoManage.mark[row][colum] > 0{
                ...
                markBallMatrix.cells[seq].image =  NSImage(named: NSImage.Name(color))
            }else{
                markBallMatrix.cells[seq].image =  NSImage(named: NSImage.Name(""))
            }
        }
    ...
    func changeTheKeyPad() {
        for cell in 0...lotoManage.keyboard.count - 1{
            ...
            numericKeypadMatrix.cells[cell].image = NSImage(named: NSImage.Name("NSStatusNone"))
        }
        for cell in 0...lotoManage.sizeOfloto - 1{
            numericKeypadMatrix.cells[cell].title = String(lotoManage.keyboard[cell])
        }
    ...
    func sportsCarDrive(_ completionHandler: () -> Void)  {
        ...
        for row in 0...4 {
            ...
            for colum in 0...lotoManage.markOfloto - 1{
                ...
                if lotoManage.mark[row][colum] > 0{
                    let color:String = "ball" + String(lotoManage.mark[row][colum] % 7)
                    DispatchQueue.main.async {self.markBallMatrix.cells[seq].image =  NSImage(named: NSImage.Name(color)) }
                }else{
                    DispatchQueue.main.async {self.markBallMatrix.cells[seq].image =  NSImage(named: NSImage.Name("")) }
                }
            }
        }
    }
