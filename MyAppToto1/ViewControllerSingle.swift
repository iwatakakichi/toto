...
class ViewControllerSingle: NSViewController, NSTableViewDataSource  {
    ...
    @IBAction func randomChance(_ sender: Any) {
        //  MARK:おまかせ選択
        //  おまかせ機能　本命・対抗・引き分けの組み合わせが同じなら、それぞれ一件をランダムに抽出
        guard stampLabel.intValue == Int32(singleScreen.stamp) else {
            print("Scrapped screen")
            return
        }
        checkScreen.stamp += 1
        temporaryTickets.removeAll()
        let seed: UInt32 = UInt32(firstOrderTickets.count - 1)
        for ticket in firstOrderTickets {
            temporaryTicket = ticket
            temporaryTicket.lastOrder = cs.outOfSelection
            temporaryTicket.buyer = String(arc4random_uniform(seed))
            temporaryTickets.append(temporaryTicket)
        }
        firstOrderTickets = temporaryTickets
        temporaryTickets.removeAll()
        buyTickets.removeAll()
        singleScreen.filtter.removeAll()
        firstOrderTickets.sort(by: {$0.buyer < $1.buyer})
        for firstOrderTicket in firstOrderTickets {
            temporaryTicket = firstOrderTicket
            let pattern = String(firstOrderTicket.major) + String(firstOrderTicket.minor) + String(firstOrderTicket.draw)
            var find = 0
            for f in singleScreen.filtter {
                if f == pattern {
                    find = 1
                }
            }
            if find != 1 {
                ...
                singleScreen.filtter.append(pattern)
            }
        }
        ...
        for ticket in firstOrderTickets{
            temporaryTicket = ticket
            for target in buyTickets {
                if temporaryTicket.firstOrder == target.firstOrder{
                    temporaryTicket.lastOrder = cs.choosing
                }
            }
            temporaryTickets.append(temporaryTicket)
