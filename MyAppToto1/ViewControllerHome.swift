...
class ViewControllerHome: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        if homeScreen.user[ch.announcement] <= 30{
            homeScreen.user[ch.announcement] += 1
            UserDefaults.standard.set(homeScreen.user, forKey: "userArray")
            guidance2Label.stringValue =  HomeSCreenGuidance1
        }
 
        var _ = initializeScreenAndTickets()
        var _ = teamNameUpload()
        imge1.image = #imageLiteral(resourceName: "img1")

    }
    
    @IBAction func acceptSale(_ sender: Any) {
        //  MARK:開催回アクセプト、購入チケットtikect.csv & トトワングランプリtotoOne.html削除
        homeScreen.user[ch.sale] = Int(acceptSaleText.intValue)
        UserDefaults.standard.set(homeScreen.user, forKey: "userArray")
        var _ = totoOnedelete()
        if homeScreen.user[ch.totoPs] > 0 && homeScreen.user[ch.totoPs] < homeScreen.user[ch.sale]{
            var _ = totoPSdelete(totoPs: homeScreen.user[ch.totoPs])
        }
        var _ = initializeScreenAndTickets()
        var _ = ticketsDelete()
